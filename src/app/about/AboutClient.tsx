...
export default function AboutClient({ content }: { content: AboutContent }) {
  const timelineEvents: TimelineEvent[] = content.about_timeline ? JSON.parse(content.about_timeline) : [
  ...
  const teamMembers: TeamMember[] = content.about_team ? JSON.parse(content.about_team) : [
        ...
        <div className="max-w-4xl mx-auto text-center">
          <Reveal>
            ...
            <p className="text-xl text-peach-deep font-sans mb-4">
              {content.about_description_1 || "Exclusive home service designed for female students and young women in Bogor."}
            </p>
            ...
            {teamMembers.map((member, index) => (
              <Reveal key={index} delay={index * 0.2}>
                <div className="group relative overflow-hidden rounded-2xl">
                  ...
                  <img src={content.about_image || member.image} alt={member.name} className="w-full h-96 object-cover transition-transform duration-700 group-hover:scale-105" />
